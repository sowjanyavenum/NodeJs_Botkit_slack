...
function insertUserSessionLog(user, sessionId, sessionSeq, entity, userQtn,intent, callback){
	...
	if(intent == 'greet' ){
		        if(user=='U5TKDN93L' && userQtn=='hi'){
						...
						userAnswer = 'we had an auditor cam ethis morning and he asked for the reports and we have provided him with the following details for user role changes' +userRoleRes+' and for sap Standard profile' +sapProfileRes+ '.He have some questions regarding them and he wants to talk to you';				
					...
					if(userQtn=='hey'||userQtn=='howdy'||userQtn=='hey there'||userQtn=='hello'||userQtn=='hi'){
					...
					else if(userQtn=='yes'||userQtn=='yeap'||userQtn=='indeed'||userQtn=='that is right'||userQtn=='great'|| userQtn=='okay'){
						...
						console.log('im 888888888888888888888888888888888888888888888888888888888888888');
						general.wait(userQtn, function(ans){
						console.log(ans);
						userAnswer = ans; 
						insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, function(res){
							console.log(res);
		...
		if(intent == 'schedule'){
				restCallMeeting.getMeetingCall(function(ans){
				userAnswer = ans;
				insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, function(res){
					console.log(res);
					callback(userAnswer);
		...
		if(intent == 'userAndSap'){
                general.userSapRoleChange(userQtn,function(ans){
				userAnswer = ans;
				insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, function(res){
					console.log(res);
					callback(userAnswer);
		...
		if(intent == 'piInterfacelookup'){
			outagePIConfig.PIConfigHelp(userQtn,entity,function(ans){
		...
		if(intent == 'outageplan'){
			...
			outagePIConfig.upgradeEccOutage(entity, function(ans){
			userAnswer = ans;
			insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, function(res){
					console.log(res);
					callback(userAnswer);
		...
		if(intent == 'upgradedetails'){
            if(entity=='start'||entity=='end'){
                outagePIConfig.upgradedetails(userQtn,entity,function(ans){
                userAnswer = ans;
                insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, function(res){
                    console.log(res);
                    callback(userAnswer);
                ...
                outagePIConfig.Conformation(userQtn, function(ans){
                userAnswer = ans; 
                insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, function(res){
                    console.log(res);
                    callback(userAnswer);
...
function insertIntoUserSessionLog(user, sessionId, sessionSeq, entity, userQtn, userAnswer, callback){
		MongoClient.connect("mongodb://localhost:27017/userHistoryHandling", function (err, db) {
		if (err) throw err;
			db.createCollection("user_session_log", function (err, res) {
				if (err) throw err;
					...
					db.collection("user_session_log").insert(myobj, function (err, res) {
						if (err) throw err;
							callback("record inserted to user_session_log");
							//callback(userAnswer);
					});
			});
	});
